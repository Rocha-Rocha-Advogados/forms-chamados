...
2. **Settings → Environments → `github-pages` → Environment variables:** `VITE_SUPABASE_URL` e
   `VITE_SUPABASE_ANON_KEY`. O `.env` não vai para o repositório, então o build lê daí — e o job de
   build declara `environment: github-pages`, que é o que dá acesso a essas variáveis. (Se um dia
   esse ambiente ganhar exigência de aprovação, mova as duas para *Repository variables* e tire o
   `environment:` do job de build.)
